            ...
            if (!isProblemImage) {
                ...
                if (c < GRID_SIZE - 1) {
                    ...
                    if (currentIdx !== 0 && rightIdx !== 0 && currentIdx !== rightIdx) {
                        ...
                        ctx.beginPath(); ctx.moveTo(x + CELL_PIXEL, y); ctx.lineTo(x + CELL_PIXEL, y + CELL_PIXEL); stroke();
                    }
                }
                if (r < GRID_SIZE - 1) {
...
function checkAnswer() {
    ...
    let hasWhite = false;
    for (let r = 0; r < GRID_SIZE; r++) {
        for (let c = 0; c < GRID_SIZE; c++) { if (userGrid[r][c] === 0) { hasWhite = true; break; } }
        if (hasWhite) break;
    }
    ...
    problemLines.forEach(pLine => {
        ...
        for (let colorId in blocks) {
            if (checkInside(pLine.start, pLine.end, blocks[colorId])) {
                trueParentColorId = colorId;
                break;
            }
        }
    ...
    for (let colorId in blocks) {
        problemLines.forEach(pLine => {
            if (checkInside(pLine.start, pLine.end, blocks[colorId])) activeBlockIds.add(colorId);
        });
    }
